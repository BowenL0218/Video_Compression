...
step_iter = int(BP_iter/Sub_step)
for i, data in enumerate(dataset):
    if i >= opt.how_many:
        break
    ...
    for _ in range(K):
        for itera in range(BP_iter):
            generated_img = model.netDecoder.forward(latent_vector*mask)
