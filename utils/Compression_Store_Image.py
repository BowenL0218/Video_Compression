...
## ADMM setting
lr = 0.05
BP_iter_list = [0,10,20,50,100,200]
BP_iter = BP_iter_list[-1]
alpha = 15
mu = 0.001

output_path = "./BP_image/"
...
if opt.feature_loss == True:
    VGG_loss = Bpgan_VGGLoss(d=40, sampling_ratio=opt.sampling_ratio, n_fft=opt.n_fft,n_mels=opt.n_mels,path="./models/VGG_Extractor_8k.pt")
    ...
    for itera in range(BP_iter):
        ...
        if itera in BP_iter_list:
            ...
            for index in range(input_label.shape[0]):
                gen_img = generated_img[index].detach().cpu().numpy()*0.5+0.5
                org_img = input_label[index].detach().cpu().numpy() *0.5+0.5
                gen_img = gen_img[0,:,:]
                org_img = org_img[0,:,:]
                gen_img = (gen_img/np.max(gen_img.flatten())*65535).astype(np.uint16)
                org_img = (org_img/np.max(org_img.flatten())*65535).astype(np.uint16)
                short_path = ntpath.basename(data['path'][index])
                name_ = os.path.splitext(short_path)[0]
                imageio.imwrite(os.path.join(output_path, name_+ "_iter_" + str(itera) + '_syn.png'), gen_img)
                if itera == 0:
